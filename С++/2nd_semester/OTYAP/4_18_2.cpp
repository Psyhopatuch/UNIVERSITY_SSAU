...
/*void sint_analizer(vector<Lex> &vec)
{
    SINT_STATES cur_state = S1;
    long unsigned int i = 0;
    while (cur_state != E1 && i < vec.size())
    {
        if (SINT_ST_MATRIX[vec[i].id][cur_state] != E1 && SINT_ST_MATRIX[vec[i].id][cur_state] != F1 && i != vec.size() - 1)
            cur_state = SINT_ST_MATRIX[vec[i].id][cur_state];
        else
        {
            Lex result;
            result.id = -1;
            int out_size = 0;
            if (SINT_ST_MATRIX[vec[i].id][cur_state] == F1 && i == vec.size() - 1)
            {
                result.str = new char[3];
                strncpy(result.str, "OK\0", 3);
                vec.push_back(result);
                return;
            }
            if (SINT_ST_MATRIX[vec[i].id][cur_state] == F1)
            {
                cur_state = SINT_ST_MATRIX[vec[i].id][cur_state];
            }
            else
            {
                result.id = i;
                for (int j = 0; j < 12; j++)
                    if (SINT_ST_MATRIX[j][cur_state] - 11)
                        out_size += 3;
                result.str = new char[out_size + 1];
                *result.str = '\0';
                for (int j = 0; j < 12; j++)
                {
                    if (SINT_ST_MATRIX[j][cur_state] - 11)
                    {
                        strcat(result.str, types[j]);
                        strcat(result.str, " \0");
                    }
                }
                // result.str[out_size + 1] = '\0';
                vec.push_back(result);
                cur_state = E1;
            }
        }
